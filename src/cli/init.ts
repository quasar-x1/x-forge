...
import { writeConfig } from '../core/config-generator';
...
async function getPackageManager(): Promise<PackageManager> {
  ...
  const selectedPackageManager = detectedPackageManager;
...
async function initForge() {
  ...
  try {
    ...
    if (installPackages) {
      console.log(
        chalk.blue(
          `ㇺ Installing packages using ${packageManager} for ${initialAnswers.projectLanguage}...`
        )
      );
      ...
      console.log(chalk.blue('Adding Forge configuration files...'));
      await writeConfig(config);
      console.log(chalk.green('Configuration files added'));
    }
